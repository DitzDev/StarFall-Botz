module.exports = (m) => {
  ...
  if (user) {
    ...
    if (!isNumber(user.donate)) user.donate = 0;
    if (!isNumber(user.jadibot)) user.jadibot = 0;
    if (!isNumber(user.expired)) user.expired = 0;
    if (!('jid' in user)) user.jid = '';
    if (!("registered" in user)) user.registered = false;
    if (!("donatur" in user)) user.donatur = false; 
  ...
  if (chat) {
    ...
    global.db.data.chats[m.chat] = {
      ...
      chat: 0,
      expired: 0,
      welcome: `Selamat datang Kak %member 👋🏻`,
      leave: `Selamat tinggal Kak %member 🥀`,
    };
